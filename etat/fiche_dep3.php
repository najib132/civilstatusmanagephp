...
$r = mysql_fetch_array($res);
  ...
  <div align="center">
    <table width="75%">
      <tr>
        <td colspan="4" bgcolor="#EFEFEF"><div align="center" class="style6">جذاذة الفرز (5/5) <span class="style8">ا لفرز</span></div></td>
      </tr>
      <tr>
...
<legend align="right" class="style39"> 	الولادات حسب مكان الإقامة  </legend>
...
//////////////////CALCUl		
  $Req =  "SELECT COUNT(code) as nbr  FROM `naissance` WHERE `annee_declaration`='".$annee."' and `date_d` BETWEEN '".$d."' and  '".$f."' and `province`=$row[id] and `sex`='M'    ";
...
$detail = mysql_fetch_assoc($res); 
 ...
 $male=$detail['nbr'];
  ...
  $Req =  "SELECT COUNT(code) as nbr  FROM `naissance` WHERE `annee_declaration`='".$annee."' and `date_d` BETWEEN '".$d."' and  '".$f."' and `province`=$row[id] and `sex`='F'    ";
$res = @mysql_query($Req,$id) or die ("<br>Probl&egrave;me d'execution de la requete <br>".mysql_error()); 
$detail = mysql_fetch_assoc($res); 
